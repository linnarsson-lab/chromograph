...
class PCA:
    ...
    """
    def __init__(self, max_n_components: int = 40, layer: str = None, test_significance: bool = True, key_depth: str = None, batch_keys: List[str] = None) -> None:
        """
        Args:
    ...
    def transform(self, ds: loompy.LoomConnection) -> np.ndarray:
        ...
        for (ix, selection, view) in ds.scan(axis=1, batch_size=self.config.params.batch_size):
            transformed[selection, :] = self.PCA.transform(view[self.layer][ds.ra.Valid==1,:].T)
            progress.update(self.config.params.batch_size)
        progress.close()
