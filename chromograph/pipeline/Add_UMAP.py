...
class Add_UMAP:
    ...
    def fit(self, ds: loompy.LoomConnection) -> None:        
        ...
        if self.feature == 'bins':
            ...
            ds.ca.UMAP_bin = UMAP(n_components=2, metric=metric_f, n_neighbors=self.config.params.k // 2, learning_rate=0.3, min_dist=0.25, init='random', verbose=True).fit_transform(decomp)
            logging.info("Generating 3D UMAP from decomposition")
            ds.ca.UMAP3D_bin = UMAP(n_components=3, metric=metric_f, n_neighbors=self.config.params.k // 2, learning_rate=0.3, min_dist=0.25, init='random', verbose=True).fit_transform(decomp)
        
            ## Plot results on manifold
            logging.info("Plotting UMAP")
            manifold(ds, os.path.join(self.outdir, f"{name}_{self.feature}_UMAP.png"), embedding = 'UMAP_bin')
            QC_plot(ds, os.path.join(self.outdir, f"{name}_{self.feature}_UMAP_QC.png"), embedding = 'UMAP_bin', attrs=['Age', 'Shortname', 'Chemistry', 'Tissue'])
            ...
            ## Plot results on manifold
            logging.info("Plotting UMAP")
            manifold(ds, os.path.join(self.outdir, f"{name}_{self.feature}_UMAP.png"), embedding = 'UMAP')
